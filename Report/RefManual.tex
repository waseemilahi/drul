...
\textbf{rand} is the function that returns a non-negative number. It either accepts a positive integer argument, in which case it returns a random number between 0(including 0 of course) and the value of the argument provided to \textbf{rand} , or no argument in which case \textbf{rand} returns either 0 or 1.
\begin{verbatim}
r = rand();
s = rand(19);
\end{verbatim}
...
The \textbf{reverse} method gives the reverse pattern from a pattern.
\textbf{reverse} doesn't take any arguments and simply returns the reverse of the original pattern.

\begin{verbatim}
  prev = pattern("101010").reverse();
\end{verbatim}
\textit{prev} is "010101".
