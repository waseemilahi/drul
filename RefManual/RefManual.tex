...
Note that the calling any of the pattern methods mentioned in this section (\textbf{next}, \textbf{prev}, \textbf{note}, and \textbf{rest}) outside the context of a mapper block is illegal.
...
p10 = map(p9) mymapper;
\end{verbatim}
With non-anonymous mappers, the names of the pattern paratmers may be used intead of the ``\$n'' variables.

A mapper function has to be defined before it is used.
